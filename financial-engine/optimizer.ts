...
/**
 * Generates prioritized liquidity recommendations for the Financial Twin using
 * deterministic rule-based logic evaluated against the 4-Tier Liquidity Waterfall.
 *
 * This is NOT a mathematical optimizer — it does not search or maximize an objective
 * function. It evaluates a fixed set of candidate actions (fill Tier 2 deficit, protect
 * working capital, trim discretionary spend, deploy Tier 4 surplus) against waterfall
 * state and returns them ranked by a fixed priority order (high > medium > low).
 *
 * Each action's constraintsChecked fields report the waterfall state at generation time,
 * not a verified post-action state — see NOTE on postActionWaterfall below.
 */
export function optimizeLiquidityDecisions(
  ...
  return {
    objective: "Rank candidate liquidity actions by waterfall deficit severity and fixed priority order",
    baselineRunwayMonths: baselineRunway,
    optimizedRunwayMonths: primaryAction.projectedRunwayAfterAction,
    recommendedActions: actions,
    primaryAction,
    // NOTE: this is the CURRENT waterfall, not recomputed after applying primaryAction.
    // Treat as "waterfall state used to generate this recommendation," not "resulting state."
    // TODO: recompute by applying primaryAction to a cloned twin and calling calculateWaterfall() again.
    postActionWaterfall: currentWaterfall,
    // NOTE: not actually verified — every candidate action above is constructed to respect
    // tier ordering by construction, but no independent check confirms this. Treat as
    // "constraints checked by construction," not "constraints independently verified."

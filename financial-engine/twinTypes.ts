...
export interface OrganizationSettings {
  ...
  customBetaOverrides?: Record<string, number>;
}

export function validateOrganizationSettings(settings?: Partial<OrganizationSettings> | null): void {
  if (!settings) return;

  if (settings.emergencyBufferMonths !== undefined) {
    if (
      typeof settings.emergencyBufferMonths !== "number" ||
      isNaN(settings.emergencyBufferMonths) ||
      !isFinite(settings.emergencyBufferMonths) ||
      settings.emergencyBufferMonths < 0
    ) {
      throw new Error(
        "Invalid OrganizationSettings: emergencyBufferMonths must be a non-negative finite number"
      );
    }
  }

  if (settings.activeCategories !== undefined) {
    if (!Array.isArray(settings.activeCategories) || settings.activeCategories.length === 0) {
      throw new Error("Invalid OrganizationSettings: activeCategories cannot be empty");
    }
  }

  if (settings.customBetaOverrides !== undefined) {
    if (typeof settings.customBetaOverrides !== "object" || settings.customBetaOverrides === null) {
      throw new Error("Invalid OrganizationSettings: customBetaOverrides must be a valid key-value mapping");
    }
    for (const [key, beta] of Object.entries(settings.customBetaOverrides)) {
      if (typeof beta !== "number" || isNaN(beta) || !isFinite(beta) || beta < 0 || beta > 5.0) {
        throw new Error(
          `Invalid OrganizationSettings: customBetaOverrides for "${key}" must be a non-negative finite number <= 5.0`
        );
      }
    }
  }
